...
STOPWORDS_LIST = STOPWORDS(version="with_conditional")
...
change_sorted.to_csv("../result/keyword_change_verified_conditional.csv", encoding="utf-8-sig")
print("분석 완료: keyword_change_verified_conditional.csv 파일이 생성되었습니다.")
...
ax[0].set_title('문재인 정부 비중 급증 담론 (TOP 10) 차별 담론 강조 분석', fontsize=15)

sns.barplot(x='증감률(%)', y=top10_park.index, data=top10_park, ax=ax[1], palette='Reds', hue=top10_park.index, legend=False)
ax[1].set_title('박근혜 정부 비중 급증 담론 (TOP 10) 차별 담론 강조 분석', fontsize=15)

plt.tight_layout()
# plt.show(
plt.savefig(
    "../result/figure_A1_tfidf_split_bar_conditional.png",
    dpi=300,
    bbox_inches="tight"
)
plt.close()
...
# plt.show()
plt.savefig(
    "../result/figure1_tfidf_importance_change.png",
    dpi=300,
    bbox_inches="tight"
)
plt.close()
